...
def upload_all(ADV=True, MASTERDB=True):
    ...
    masterdb_result = {"files": [], "remote_path": MASTERDB_REMOTE_PATH}
    ...
    if MASTERDB:
        ...
        masterdb_result["files"] = _upload_pipeline(MASTERDB_DRIVE_PATH, MASTERDB_REMOTE_PATH)
...
def _download_masterdb(bFullUpdate):
    if bFullUpdate:
        ...
        return Helper_GetFilesFromDir(MASTERDB_DRIVE_PATH, ".xlsx")
    else:
        check_result = rclone.check(MASTERDB_REMOTE_PATH, MASTERDB_DRIVE_PATH)
        changed = Helper_GetFilesFromDirByCheck(check_result, MASTERDB_DRIVE_PATH, ".xlsx")
        rclone.copy(MASTERDB_REMOTE_PATH, MASTERDB_DRIVE_PATH)
        return changed
...
def _download_generic(bFullUpdate):
    ...
    # Sync individual generic files
    for file in GENERIC_FILE_LIST:
        remote_file = GENERIC_REMOTE_PATH + file
        if bFullUpdate:
            rclone.copy(remote_file, GENERIC_DRIVE_PATH)
            drive_file_paths += [(GENERIC_DRIVE_PATH + file, file, os.path.basename(file))]
        else:
            check_result = rclone.check(remote_file, GENERIC_DRIVE_PATH)
