...
from .helper import (
    Helper_GetFilesFromDir, Helper_GetUpdatedFiles,
...
def TxtToXlsx(input_path: str, output_path: str, file_name: str) -> list[str]:
    ...
    if dataframe.empty:
        LOG_DEBUG(2, f"Skip empty file: {file_name}")
        return []
...
def XlsxToTxt_parallels(obj):
    ...
    try:
        ...
        error_msg = str(e)
        if "[SKIP_EMPTY]" not in error_msg:
...
def _internalGetOutputPath(filename: str) -> str:
    """Extract output folder name from an Idoly Pride ADV filename."""
    # Example: adv_main_01_01.txt -> main
    # Example: adv_userhbd_2024.txt -> userhbd
    splitted_name = filename[4:-4].split("_")
    if len(splitted_name) > 0:
        folder_name = splitted_name[0]
        # Handle special cases if any, but currently just return the first segment
        return folder_name
    return "etc"


def _filter_adv_files(file_paths):
    """Apply whitelist and map to (input_path, output_xlsx_path, filename)."""
    VALID_CATEGORIES = ['bond', 'card', 'main', 'event', 'group', 'hbd', 'love', 'userhbd']
    file_list = []
    for abs_path, rel_path, filename in file_paths:
        foldername = _internalGetOutputPath(filename)
        if foldername not in VALID_CATEGORIES:
            continue
        input_path = rel_path
...
def UpdateOriginalToDrive(bFullUpdate=False):
    """Update: TXT → Google Drive XLSX."""
    last_update_date = load_cache_date(ADV_CACHE_FILE)
    if last_update_date:
        LOG_DEBUG(2, f"Load update date {last_update_date}")

    if not bFullUpdate and last_update_date is not None:
        LOG_DEBUG(2, "Check git diff")
        original_file_paths = Helper_GetUpdatedFiles(
            last_update_date, GIT_ADV_PATH, ".txt", "adv_"
        )
    else:
        LOG_INFO(2, "No cache or full update requested, scanning all files")
